 ...
 */
@Repository
public interface ITerminalCommandRepository extends R2dbcRepository<TerminalCommand, String> {
	 ...
	 * @return 可见的命令列表
	 */
	@Query("SELECT * FROM plugin.terminal_commands WHERE enabled = TRUE AND (hidden IS NULL OR hidden = FALSE)")
	 ...
	 * @return 是否存在
	 */
	@Query("SELECT EXISTS(SELECT 1 FROM plugin.terminal_commands WHERE name = :name)")
