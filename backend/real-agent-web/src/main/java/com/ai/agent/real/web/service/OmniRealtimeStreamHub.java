...
import com.ai.agent.real.contract.service.playground.roleplay.PlaygroundRoleplayRoleService;
...
/**
 * Realtime streaming session hub for Route B: - Manage Omni realtime conversations per
 * sessionId - Receive PCM frames (16k, mono, 16-bit LE) via WebSocket and forward to SDK
 * - Rely on enableTurnDetection(true) and input audio transcription - Broadcast events to
 * SSE subscribers with OmniSseEvent
 *
 * @author han
 * @time 2025/9/26 1:38
 */
...
public class OmniRealtimeStreamHub {
	...
	private SessionState startSession(String sessionId, String roleId) {
		...
		OmniRealtimeParam param = OmniRealtimeParam.builder().model(model).apikey(apiKey).build();
		...
		String systemPrompt = String.format("system: 你要扮演%s，请始终使用中文回复, 该提示词将作为系统提示词, 不需要回复", setup.getPrompt());

		OmniRealtimeConversation conversation = new OmniRealtimeConversation(param, new OmniRealtimeCallback() {
			@Override
			public void onOpen() {
				log.info("[hub] omni open sid={}", sessionId);
			}
			...
			public void onEvent(JsonObject message) {
				...
				switch (event.getType()) {
					case SESSION_CREATED:
						...
						if (!systemPromptSet.getAndSet(true)) {
							try {
								...
								state.conversationRef.get()
									.createResponse(systemPrompt, Arrays.asList(OmniRealtimeModality.TEXT));
					...
					case INPUT_AUDIO_BUFFER_COMMITTED:
						// Buffer committed by server - VAD will handle response
						// automatically
						log.info("[hub] audio buffer committed sid={} - waiting for VAD to trigger response",
								sessionId);
						break;
		...
		try {
			...
			OmniRealtimeConfig cfg = OmniRealtimeConfig.builder()
				...
				.parameters(Map.of("smooth_output", "true", "system_prompt", systemPrompt))
				// 降低敏感度，减少噪音干扰
	...
	private OmniServerEvent parseServerEvent(JsonObject message) {
		...
		if (message != null) {
			...
			if (evt.getType() == OmniServerEventType.SESSION_CREATED && message.has("session")
					&& message.get("session").isJsonObject()) {
