...
public class ToolUtils {
	...
	public static boolean hasTaskDone(ChatResponse response) {
		...
		return toolCalls.stream()
			...
			.anyMatch(toolCall -> NounConstants.TASK_DONE.equals(toolCall));
	}
	...
	public static boolean hasTaskDoneNative(ChatResponse response) {
		...
		return response.getResult()
			...
			.stream()
			.filter(Objects::nonNull)
			.anyMatch(toolCall -> NounConstants.TASK_DONE.equals(toolCall.name()));
	}
	// public static boolean hasTaskDone(ChatResponse response) {
	//
	// List<Generation> generations = response.getResults();
	 ...
	 * 执行工具调用
	 */
	// public Object executeToolCall(ToolCall toolCall, AgentContext context,
	// List<AgentTool> availableTools) throws ToolException {
		...
		for (ToolCallback toolCallback : toolCallbacks) {
			AgentTool agentTool = new AgentTool() {
				...
				public ToolSpec getSpec() {
					return new ToolSpec().setName(toolCallback.getToolDefinition().name())
						.setDescription(toolCallback.getToolDefinition().description())
						.setInputSchema(toolCallback.getToolDefinition().inputSchema())
						.setCategory(MCP)
						.setMcpToolSpec(McpToolSpec.builder()
							.server(((McpAsyncClient) ReflectionUtils.findField(toolCallback.getClass(), "mcpClient")
								.get(toolCallback)).getServerInfo().name())
							// 去除 mcpAsyncClient.getClientInfo().name() 里的最后一个 " - " +
							// mcpAsyncClient.getServerInfo().name()
							.client(((McpAsyncClient) ReflectionUtils.findField(toolCallback.getClass(), "mcpClient")
								.get(toolCallback)).getClientInfo().name())
							.build());
				}
