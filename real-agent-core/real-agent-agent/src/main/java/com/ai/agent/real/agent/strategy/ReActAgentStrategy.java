...
public class ReActAgentStrategy implements AgentStrategy {
	...
	public Flux<AgentExecutionEvent> executeStream(String task, List<Agent> agents, AgentContext context) {
		...
		return Flux.concat(
				...
				Flux.range(1, MAX_ITERATIONS)
					...
					.concatWith(Flux
						.defer(() -> finalAgent.executeStream(task, createAgentContext(context, FinalAgent.AGENT_ID))
							.transform(FluxUtils.handleContext(context, FinalAgent.AGENT_ID))
							.concatWith(Flux.just((taskCompleted.get() || context.isTaskCompleted())
	...
	private Flux<AgentExecutionEvent> executeReActIteration(String task, AgentContext context, int iteration) {
		...
		return Flux.concat(
				...
				Flux.defer(() -> {
					...
					return FluxUtils.stage(thinkingAgent.executeStream(task, thinkingCtx), context,
							...
							evt -> log.debug("[EVT/THINK/{}] type={}, msg={}...", iteration,
									evt != null ? evt.getType() : null,
									safeHead(evt != null ? evt.getMessage() : null, 256)),
				...
				Flux.defer(() -> {
					...
					return FluxUtils.stage(observationAgent.executeStream(task, observingCtx)
						// 观察阶段可能会发出 DONE 事件：为了让 FinalAgent 统一收尾，不把该 DONE 直接透传到前端
						.filter(evt -> evt == null
								|| evt.getType() == null || !"DONE".equals(evt.getType().toString())),
							context, ObservationAgent.AGENT_ID,
		...
		// return Flux.empty();
		// }
		// return observationAgent.executeStream(task, createAgentContext(context,
	...
	public AgentResult execute(String task, List<Agent> agents, AgentContext context) {
		...
		// conversationHistory.append("可用工具:
		// ").append(getToolsDescription(availableTools)).append("\n\n");
		//
		// for (int iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
		// log.debug("ReAct循环第{}轮开始", iteration);
		//
		// // 1. 思考阶段 (Thought)
		// AgentResult thoughtAgentResult =
		// generateThought(conversationHistory.toString(), availableTools);
		// String thought = thoughtAgentResult.getResult();
		// conversationHistory.append("思考").append(iteration).append(":
		...
		//
		// // 2. 行动阶段 (Action)
		// AgentResult actionAgentResult =
		// generateActionWithTools(conversationHistory.toString(), chatModel,
		// availableTools);
		// String action = actionAgentResult.getResult();
		// conversationHistory.append("行动").append(iteration).append(":
		// ").append(action).append("\n");
		//
		// // 3. 观察阶段 (Observation)
		// String observation = executeActionWithTools(action, context, availableTools);
		// conversationHistory.append("观察").append(iteration).append(":
