...
public class ThinkingAgent extends Agent {
	...
	private final String SYSTEM_PROMPT = """
			...
			// 2. 实例中的 Thought, user 等 只是说明该输出的角色, 并非真正的输出, 你只负责Thought节点的输出
			### 示例1
			user: "2024 年巴黎奥运会的主会场是什么？能容纳多少观众？

			Thought: 我需要回答关于2024年巴黎奥运会主会场的问题。首先，我应该确认主会场的名称和容量, 需要调用 search 工具查找2024年巴黎奥运会主会场的信息，包括名称和观众容量。
			Action: search("2024年巴黎奥运会主会场 名称 容量")
			Observation: 2024年巴黎奥运会的主会场是法兰西体育场(Stade de France)，可容纳80,698名观众。

			Thought: 搜索结果提供了主会场名称和容量信息，但我应该调用 search 工具去确认这些信息的准确性。
			Action: search("法兰西体育场 2024年奥运会 观众容量")
			Observation: 法兰西体育场经过改造后，在2024年奥运会期间的观众容量为80,698人，是开幕式、闭幕式和田径比赛的举办地。

			Thought: 现在我有了准确的信息，可以整理成完整的回答, 应该调用 任务结束 工具。
			Action: 调用任务结束工具
			...
			Thought: 这是一个复杂的学习规划问题，需要分步骤分析并制定详细的学习计划, 首先需要了解用户的背景和目标，然后制定个性化的学习路径。用户需要一个3个月的机器学习学习计划，目标是找到工作。我需要考虑：学习基础、实践项目、求职准备等方面, 我应该先调用 search 工具去了解机器学习的学习路径和求职要求。
			Action: search("机器学习学习路径 3个月 求职")
			Observation: 机器学习学习路径通常包括：数学基础→编程技能→机器学习算法→深度学习→项目实践→求职准备。成功求职需要：扎实的理论基础、实际项目经验、技术面试准备。

			Thought: 需要调用 plan 工具去制定详细的月度学习计划。
			Action: plan("第一个月：数学基础和编程技能")
			Observation: 第一月重点：Python编程、数据结构、数学基础（线性代数、微积分、概率统计）、数据分析库（pandas、numpy、matplotlib）

			Thought: 需要调用 plan 工具去制定第二个月的学习内容。
			Action: plan("第二个月：机器学习算法和实践")
			Observation: 第二月重点：监督学习算法（线性回归、逻辑回归、决策树、随机森林、SVM）、无监督学习、模型评估、scikit-learn库、第一个项目实践

			Thought: 需要调用 plan 工具去制定第三个月的学习内容。
			Action: plan("第三个月：深度学习和求职准备")
			Observation: 第三月重点：神经网络基础、深度学习框架（TensorFlow/PyTorch）、深度学习应用、项目实战、简历准备、面试刷题

			Thought: 需要调用 calculate 工具 计算具体的每日学习时间安排。
			Action: calculate("假设每天学习6小时，每周学习6天，3个月共72天，总计432小时")

			Thought: 需要调用 schedule 工具 进行学习时间的分配到各个主题。
			Action: schedule("时间分配：数学基础60小时、编程技能80小时、机器学习算法120小时、深度学习80小时、项目实践60小时、求职准备32小时")

			Thought: 需要调用 search 工具搜索具体的学习资源推荐。
			Action: search("机器学习最佳学习资源 2024年")
			Observation: 推荐资源：Coursera Andrew Ng机器学习课程、Stanford CS229、Python for Data Science Handbook、Scikit-learn官方文档、TensorFlow/PyTorch教程、Kaggle竞赛平台

			Thought: 需要调用 project 工具来规划具体的项目实践。
			Action: project("项目规划：1.数据分析项目（用pandas分析数据集）2.机器学习项目（预测模型）3.深度学习项目（图像识别/NLP）4.综合项目（端到端ML系统）")

			Thought: 需要调用 search 工具去了解当前机器学习的求职市场需求。
			Action: search("机器学习工程师求职要求 技能需求 2024年")
			Observation: 主要技能需求：Python编程、机器学习算法、深度学习框架、数据处理、模型部署、项目经验、沟通能力。热门工具：Scikit-learn、TensorFlow、PyTorch、Docker、Kubernetes、AWS/Azure/GCP。

			Thought: 需要调用 search 工具用于搜索准备面试常见问题和算法题。
			Action: search("机器学习面试题 算法题 2024年")
			Observation: 常见面试内容：机器学习理论基础、算法原理、项目经验、编程能力、系统设计、开放性问题。推荐刷题平台：LeetCode、HackerRank、Kaggle。

			Thought: 现在所有任务已经完成, 任务可以结束了, 应该对所有信息进行整合, 制定完整的学习计划
			Action: 调用任务结束工具
	...
	public Flux<AgentExecutionEvent> executeStream(String task, AgentContext context) {
		...
		Prompt prompt = AgentUtils.buildPromptWithContext(this.availableTools, context, SYSTEM_PROMPT,
				thinkingPrompt);
